...
func (b *bareMetalInventory) UpdateHostInstallProgress(ctx context.Context, params installer.UpdateHostInstallProgressParams) middleware.Responder {
	...
	msg := fmt.Sprintf("Host %s in cluster %s reached installation step %s", host.ID, host.ClusterID, params.HostInstallProgressParams.ProgressInfo)

	if params.HostInstallProgressParams.ProgressInfo != "" {
		msg += fmt.Sprintf(": %s", params.HostInstallProgressParams.ProgressInfo)
	}
